...
class PianoDetector:
    """
    Detects the piano sheet, keys, and dots (used for key-press detection) from an image.
    """

    # Note: The region-of-interest ratios and key mappings are defined as constants.
    ...
    # Map black key visual order (0-4) to note indices.
    BLACK_KEY_VISUAL_MAP = {i: note for i, note in enumerate(BLACK_KEY_NOTE_INDICES)}
    
    def detect_sheet(self, image: np.ndarray):
        """
        Detect the four corners of the piano sheet in the given image.
        Returns a tuple (ordered_corners, bounding_rect) or (None, None) if not found.
        """
            ...
            for contour in candidates:
                ...
                if len(approx) == 4:
                    ...
                    if 1.5 < aspect_ratio < 6.0:
                        ...
                        # Order corners robustly: top-left, top-right, bottom-right, bottom-left.
                        ...
                        # Additional robust sort based on y then x.
                        ...
                        ordered_corners_robust = np.array([top_sorted[0], top_sorted[1], bottom_sorted[1], bottom_sorted[0]], dtype=np.int32)
                        return ordered_corners_robust, cv2.boundingRect(contour)

            # Fallback: try using the largest contour approximation.
    ...
    def detect_keys(self, warped_image: np.ndarray):
        """
        Detect white and black piano keys from a perspective-corrected image.
        Returns (white_keys, black_keys), where each key is represented as (x, y, w, h).
        """
    ...
    def detect_dots(self, warped_image: np.ndarray, white_keys: list, black_keys: list):
        """
        Detect dots on the keys that help determine if a key press is happening.
        Returns (white_dots, black_dots) where each dot is (x, y, key index).
        """
        ...
        # Settings for white key (dark dot) detection.
        block_size = 11
        C_val = 2
        kernel = np.ones((3, 3), np.uint8)
        # Settings for black key (white dot) detection.
        block_size_black = 15
        C_val_black = -2
        kernel_black = np.ones((3, 3), np.uint8)
        
        # Process black keys for white dot detection.
        ...
        # Process white keys for dark dot detection.
    ...
    def detect_pressed_key(self, fingertip: tuple, white_keys: list, black_keys: list, margin: float = 0.05):
        """
        Given a fingertip (x, y) on the warped image, determine which piano key is being pressed.
        Returns the note index (from white or black keys) or None.
        """
        candidates = []
